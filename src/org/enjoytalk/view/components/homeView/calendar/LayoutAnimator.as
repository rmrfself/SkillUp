		...
		public function updateLayout():void
		{
			...
			if (_priorityInvalid)
			{
				...
				for (var aChild:*in _targets)
				{
					...
					if (autoPrioritize)
					{
						target.priority=(target.state == "removed") ? 0 : (target.state == "added") ? 2 : 1;
					}
			...
			for (var i:int=_orderedTargets.length - 1; i >= 0; i--)
			{
				...
				else if (target.animate == false || (Math.abs(m.tx - target.x) < TRANSLATE_TOLERANCE && Math.abs(m.ty - target.y) < TRANSLATE_TOLERANCE && Math.abs(m.a - target.scaleX) < SCALE_TOLERANCE && Math.abs(m.d - target.scaleY) < SCALE_TOLERANCE && Math.abs(item.width - target.unscaledWidth) < SIZE_TOLERANCE && Math.abs(item.alpha - target.alpha) < ALPHA_TOLERANCE && Math.abs(item.height - target.unscaledHeight) < SIZE_TOLERANCE))
				{
					m.tx=target.x;
					m.ty=target.y;
					m.a=target.scaleX;
				...
				{
					item.setActualSize(prevWidth + _animationSpeed * (target.unscaledWidth - prevWidth), prevHeight + _animationSpeed * (target.unscaledHeight - prevHeight));
				}
			}
