package org.enjoytalk.view.components.homeView.calendar
{
	...
	public class DateRange
	{
		...
		{
			this.start=start;
			this.end=(end != null) ? end : (start != null) ? new Date(start.getTime()) : null;
		}
		...
		{
			return new DateRange(new Date(Math.max(start.getTime(), rhs.start.getTime())), new Date(Math.min(end.getTime(), rhs.end.getTime())));
