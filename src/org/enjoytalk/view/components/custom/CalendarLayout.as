...
package org.enjoytalk.view.components.custom
{
	...
	[Style(name="dayIndicatorSkin", type="Class", inherit="no")]

	[Style(name="dataIndicatorSkin", type="Class", inherit="no")]
	...
	[Style(name="dataStyleName", type="String", inherit="no")]
	...
	public class CalendarLayout extends UIComponent implements IFocusManagerComponent, IFontContextComponent
	{
		...
		mx_internal var dataBlocksArray:Array=[];
		...
		private var _extraData:Dictionary=new Dictionary();
		...
		private var dayBackgroundContainer:UIComponent;

		private var dayIndicator:UIComponent;

		private var dayBgArrays:Array;

		private var extraDataArrays:Array;

		private var extraDataContainer:UIComponent;
		/**
		 *  @private
		 */
		...
		public function set dayNames(value:Array):void
		{
			...
			_dayNames=value != null ? value : resourceManager.getStringArray("controls", "dayNamesShortest");
		...
		public function set disabledRanges(value:Array):void
		{
			...
			for (var i:int=0; i < _disabledRanges.length; i++)
			{
				if (_disabledRanges[i] is Date)
				{
					...
					_disabledRanges[i]=new Date(value[i].getFullYear(), value[i].getMonth(), value[i].getDate());
				}
				else if (_disabledRanges[i] is Object)
				{
					...
					if (!_disabledRanges[i].rangeStart && _disabledRanges[i].rangeEnd)
					{
						disabledRangeMode[i]=3;
					}
					else if (_disabledRanges[i].rangeStart && !_disabledRanges[i].rangeEnd)
					{
		...
		public function set selectableRange(value:Object):void
		{
			...
			if (value is Date)
			{
				...
				_selectableRange=new Date(value.getFullYear(), value.getMonth(), value.getDate());
			...
			else if (value is Object)
			{
				...
				if (!value.rangeStart && value.rangeEnd)
				{
					...
					if (todaysYear <= _selectableRange.rangeEnd.getFullYear())
					{
						...
						else if (todaysMonth <= _selectableRange.rangeEnd.getMonth())
						{
				...
				else if (!value.rangeEnd && value.rangeStart)
				{
					...
					if (todaysYear >= _selectableRange.rangeStart.getFullYear())
					{
						...
						else if (todaysMonth >= _selectableRange.rangeStart.getMonth())
						{
							callMonth=todaysMonth;
							callYear=todaysYear;
						}
				...
				else if (value.rangeStart && value.rangeEnd)
				{
					...
					if (todaysDate >= _selectableRange.rangeStart && todaysDate <= _selectableRange.rangeEnd)
					{
						callMonth=todaysMonth;
						callYear=todaysYear;
					}
		...
		public function get selectedRanges():Array
		{
			if (_selectableRange)
			{
				switch (selRangeMode)
				{
					...
					case 3:
					{
						removeRangeFromSelection(_selectableRange.rangeEnd, _selectableRange.rangeStart);
						break;
					}
			...
			for (i=0; i < _disabledRanges.length; i++)
			{
				switch (disabledRangeMode[i])
				{
					...
					case 4:
					{
						removeRangeFromSelection(_disabledRanges[i], _disabledRanges[i]);
						break;
					}
				}
			}
		...
		*/
		...
		protected function createExtraData():void
		{
			if (extraDataContainer == null)
			{
				extraDataContainer=new UIComponent();
				addChildAt(extraDataContainer, 1);
			}
			extraDataArrays=new Array(7);
			var childIndex:Number=0;
			for (var col:Number=0; col < 7; col++)
			{
				extraDataArrays[col]=new Array(7);
				for (var row:Number=0; row < 7; row++)
				{
					var dataIndicatorClass:Class=getStyle("dataIndicatorSkin");
					if (!dataIndicatorClass)
						dataIndicatorClass=DateChooserIndicator;
					var dataIndicator:IFlexDisplayObject=extraDataArrays[col][row]=IFlexDisplayObject(new dataIndicatorClass());
					extraDataContainer.addChildAt(DisplayObject(dataIndicator), childIndex++);
					dataIndicator.visible=false;
				}
			}
		}

		protected function createDaysBackground():void
		{
			if (dayBackgroundContainer == null)
			{
				dayBackgroundContainer=new UIComponent();
				addChildAt(dayBackgroundContainer, 0);
			}
			dayBgArrays=new Array(7);
			var childIndex:Number=0;
			for (var col:Number=0; col < 7; col++)
			{
				dayBgArrays[col]=new Array(7);
				for (var row:Number=0; row < 7; row++)
				{
					var dayIndicatorClass:Class=getStyle("dayIndicatorSkin");
					if (!dayIndicatorClass)
						dayIndicatorClass=DateChooserIndicator;
					var dayIndicator:IFlexDisplayObject=dayBgArrays[col][row]=IFlexDisplayObject(new dayIndicatorClass());
					dayBackgroundContainer.addChildAt(DisplayObject(dayIndicator), childIndex++);
					dayIndicator.visible=false;
				}
			}
		}
		...
		override protected function commitProperties():void
		{
			...
			if (selectedMonthYearChanged)
			{
				...
				var proposedDate:Date=new Date(_proposedDisplayedYear == -1 ? _displayedYear : _proposedDisplayedYear, _proposedDisplayedMonth == -1 ? _displayedMonth : _proposedDisplayedMonth);
		...
		override protected function measure():void
		{
			...
			measuredWidth=paddingLeft + horizontalGap * 6 + cellWidth * 7 + paddingRight;
			measuredHeight=verticalGap * 6 + cellHeight * 7 + paddingBottom + paddingTop;
		...
		mx_internal function createDataLabels(childIndex:int):void
		{
			var dataStyleName:Object=getStyle("dataStyleName");
			dataBlocksArray=new Array(7);
			for (var columnIndex:int=0; columnIndex < 7; columnIndex++)
			{
				dataBlocksArray[columnIndex]=new Array(7);
				for (var rowIndex:int=0; rowIndex < 7; rowIndex++)
				{
					var label:IUITextField=dataBlocksArray[columnIndex][rowIndex]=IUITextField(createInFontContext(UITextField));
					label.selectable=false;
					label.ignorePadding=true;
					if (childIndex == -1)
					{
					...
					label.styleName=dataStyleName ? dataStyleName : this;
				}
			}
		}
		...
		/**
		 * This function is created by xanderzhang , copyright reserved.
		 *
		 *
		 * **/
		public function setExtraMonthData(data:Array):void
		{
			...
			{
				var dataKey:String=[obj.year, obj.month, obj.day].join("-");
				this.extraData[dataKey]=obj.text;
				break;
			}
			invalidateDisplayList();
		}
		...
		mx_internal function setSelectedMonthAndYear(monthVal:int=-1, yearVal:int=-1):void
		{
			...
			var dayBgIndicator:IFlexDisplayObject;
			for (columnIndex=0; columnIndex < offset; columnIndex++)
			{
				...
				// day indicator --- xander
				dayBgIndicator=dayBgArrays[columnIndex][rowIndex];
				dayBgIndicator.visible=true;
				dayBgIndicator.alpha=0.2;
				dayBgIndicator.move(dayBlocksArray[columnIndex][rowIndex].x, dayBlocksArray[columnIndex][rowIndex].y);
				removeSelectionIndicator(columnIndex, rowIndex);
				hiddenDataFields(columnIndex, rowIndex);
			}
			...
			for (dayNumber=1; dayNumber <= daysInMonth; dayNumber++)
			{
				...
				dayBgIndicator=dayBgArrays[columnIndex][rowIndex];
				dayBgIndicator.visible=true;
				dayBgIndicator.alpha=(columnIndex == 0 ? 0.4 : 1);
				dayBgIndicator.move(todayLabel.x, todayLabel.y);
				// Enable the day.
				...
				var dataKey:String=[cellDate.fullYear, (cellDate.getMonth() + 1), cellDate.getDate()].join("-");
				var dataValue:*=this.extraData[dataKey];
				var dataLabel:IUITextField=dataBlocksArray[columnIndex][rowIndex];
				var dataIndicator:IFlexDisplayObject=IFlexDisplayObject(extraDataArrays[columnIndex][rowIndex]);
				var dataOffsetX:Number=28;
				var dataOffsetY:Number=8;
				if (dataValue != null)
				{
					dataIndicator.move(todayLabel.x + dataOffsetX, todayLabel.y + dataOffsetY);
					dataIndicator.visible=true;
					dataLabel.text=String(dataValue);
					dataLabel.move(todayLabel.x + dataOffsetX, todayLabel.y + dataOffsetY);
			...
			for (i=offset + daysInMonth; i < 42; i++)
			{
				...
				dayBgIndicator.alpha=0.2;
				dayBgIndicator.move(dayBlocksArray[columnIndex][rowIndex].x, dayBlocksArray[columnIndex][rowIndex].y);
				if (dataBlocksArray[columnIndex][rowIndex])
				{
					dataBlocksArray[columnIndex][rowIndex].visible=false;
				}
			}
		...
		private function hiddenDataFields(columnIndex:Number=-1, rowIndex:Number=-1):void
		{
			if (dataBlocksArray[columnIndex][rowIndex].visible)
			{
				dataBlocksArray[columnIndex][rowIndex].text="";
				dataBlocksArray[columnIndex][rowIndex].visible=false;
				extraDataArrays[columnIndex][rowIndex].visible=false;
			}
		}
		...
		{
			var change:CalendarLayoutChangeEvent=new CalendarLayoutChangeEvent(CalendarLayoutChangeEvent.CHANGE);
		...
		mx_internal function isDateInRange(value:Date, dateRange:Object, rangeMode:int, ignoreDay:Boolean=false):Boolean
		{
			...
			if (dateRange)
			{
				...
				switch (rangeMode)
				{
					case 1:
					{
						if (value < dateRange.rangeStart || value > dateRange.rangeEnd)
						{
							result=false;
						}
						break;
		...
		mx_internal function addToSelected(newDate:Date, range:Boolean=false):void
		{
			...
			if (range == false)
			{
				...
				_selectedRanges[selectedRangeCount].rangeStart=new Date(newDate);
		...
		mx_internal function isSelected(newDate:Date):Boolean
		{
			...
			{
				if (newDate >= _selectedRanges[i].rangeStart && newDate <= _selectedRanges[i].rangeEnd)
				{
					return true;
				}
			}
		...
		mx_internal function addSelectionIndicator(columnIndex:int, rowIndex:int):void
		{
			if (!selectionIndicator[columnIndex][rowIndex])
			{
				...
				var selectionIndicatorClass:Class=getStyle("selectionIndicatorSkin");
				if (!selectionIndicatorClass)
					selectionIndicatorClass=DateChooserIndicator;
				selectionIndicator[columnIndex][rowIndex]=IFlexDisplayObject(new selectionIndicatorClass());

				if (isDateChooserIndicator(selectionIndicator[columnIndex][rowIndex]))
					Object(selectionIndicator[columnIndex][rowIndex]).indicatorColor="selectionColor";
			...
			dayBgArrays[columnIndex][rowIndex].visible=false;
		 ...
		 *  @private
		 */
		mx_internal function removeSelectionIndicator(columnIndex:int, rowIndex:int):void
		{
			if (selectionIndicator[columnIndex][rowIndex])
			{
				removeChild(selectionIndicator[columnIndex][rowIndex]);
				selectionIndicator[columnIndex][rowIndex]=null;
				dayBgArrays[columnIndex][rowIndex].visible=true;
			}
		}
		...
		override protected function keyDownHandler(event:KeyboardEvent):void
		{
			...
			*/
			...
			for (var i:uint=0; i < 31; i++)
			{
				...
				else if (lastSelectedDate && event.shiftKey && (keyCode == Keyboard.PAGE_UP || keyCode == Keyboard.PAGE_DOWN))
				{
					selChanged=true;
				}

				else if (lastSelectedDate && (keyCode == 189 || keyCode == 187)) // for year - and +
				{
					selChanged=true;
				}

				if (keyCode >= Keyboard.PAGE_UP && keyCode <= Keyboard.DOWN)
				{
		...
		public function get extraData():Dictionary
		{
			return _extraData;
		}

		public function set extraData(value:Dictionary):void
		{
			_extraData=value;
		}
