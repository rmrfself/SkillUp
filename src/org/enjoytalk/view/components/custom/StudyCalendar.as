...
package org.enjoytalk.view.components.custom
{
	...
	import org.enjoytalk.view.components.custom.CalendarLayout;
	import org.enjoytalk.view.components.homeView.calendar.CalendarData;
	 ...
	 *  @productversion Flex 3
	 */
	public class StudyCalendar extends UIComponent implements IFocusManagerComponent, IFontContextComponent
	{
		...
		public function set firstDayOfWeek(value:Object):void
		{
			...
			_firstDayOfWeek=value != null ? int(value) : resourceManager.getInt("controls", "firstDayOfWeek");
		...
		public function set monthSymbol(value:String):void
		{
			...
			_monthSymbol=value != null ? value : resourceManager.getString("SharedResources", "monthSymbol");
		...
		private static var _nextMonthStyleFilters:Object={"highlightAlphas": "highlightAlphas", "nextMonthUpSkin": "nextMonthUpSkin", "nextMonthOverSkin": "nextMonthOverSkin", "nextMonthDownSkin": "nextMonthDownSkin", "nextMonthDisabledSkin": "nextMonthDisabledSkin", "nextMonthSkin": "nextMonthSkin", "repeatDelay": "repeatDelay", "repeatInterval": "repeatInterval"};
		...
		private static var _nextYearStyleFilters:Object={"highlightAlphas": "highlightAlphas", "nextYearUpSkin": "nextYearUpSkin", "nextYearOverSkin": "nextYearOverSkin", "nextYearDownSkin": "nextYearDownSkin", "nextYearDisabledSkin": "nextYearDisabledSkin", "nextYearSkin": "nextYearSkin", "repeatDelay": "repeatDelay", "repeatInterval": "repeatInterval"};
		...
		private static var _prevMonthStyleFilters:Object={"highlightAlphas": "highlightAlphas", "prevMonthUpSkin": "prevMonthUpSkin", "prevMonthOverSkin": "prevMonthOverSkin", "prevMonthDownSkin": "prevMonthDownSkin", "prevMonthDisabledSkin": "prevMonthDisabledSkin", "prevMonthSkin": "prevMonthSkin", "repeatDelay": "repeatDelay", "repeatInterval": "repeatInterval"};
		...
		private static var _prevYearStyleFilters:Object={"highlightAlphas": "highlightAlphas", "prevYearUpSkin": "prevYearUpSkin", "prevYearOverSkin": "prevYearOverSkin", "prevYearDownSkin": "prevYearDownSkin", "prevYearDisabledSkin": "prevYearDisabledSkin", "prevYearSkin": "prevYearSkin", "repeatDelay": "repeatDelay", "repeatInterval": "repeatInterval"};
		...
		public function set yearSymbol(value:String):void
		{
			...
			_yearSymbol=value != null ? value : resourceManager.getString("controls", "yearSymbol");
		...
		{
			if (StudyCalendar.createAccessibilityImplementation != null)
				StudyCalendar.createAccessibilityImplementation(this);
		}
		...
		override protected function measure():void
		{
			...
			headerHeight=Math.ceil(Math.max(monthSkinHeight, monthDisplay.getExplicitOrMeasuredHeight())) + SKIN_HEIGHT_PAD * 2;

			//monthDisplay.width = dateGrid.getExplicitOrMeasuredWidth() - allPads - yearWidth;

			measuredWidth=Math.max(dateGrid.getExplicitOrMeasuredWidth() + borderThickness * 2, monthDisplay.width + yearDisplay.getExplicitOrMeasuredWidth() + HEADER_WIDTH_PAD + yearSkinWidth + YEAR_BUTTONS_PAD + (monthSkinWidth + SKIN_WIDTH_PAD * 2) * 2);
		...
		override protected function updateDisplayList(unscaledWidth:Number, unscaledHeight:Number):void
		{
			...
			if (swapOrder)
				...
				yearX=w - (monthSkinWidth + HEADER_WIDTH_PAD + yearSkinWidth + YEAR_BUTTONS_PAD) + borderThickness;
			var dateHeight:Number=borderThickness + (headerHeight - monthHeight) / 2;

			var allPads:Number=HEADER_WIDTH_PAD + yearSkinWidth + (monthSkinWidth + SKIN_WIDTH_PAD * 2) * 2;
			...
			borderContext.drawRoundRect(0, 0, unscaledWidth, unscaledHeight, cornerRadius * 2, cornerRadius * 2);
			borderContext.drawRoundRect(borderThickness, borderThickness, unscaledWidth - borderThickness * 2, unscaledHeight - borderThickness * 2, cornerRadius * 2, cornerRadius * 2);
			...
			else
			{
				...
				isHalo=borderSkinClass && (getQualifiedClassName(borderSkinClass).indexOf("skins.halo") >= 0);
			}
			...
			borderContext.drawRoundRect(borderThickness, borderThickness, w, h, cornerRadius > 0 ? (cornerRadius - 1) * 2 : 0, cornerRadius > 0 ? (cornerRadius - 1) * 2 : 0);
			borderContext.endFill();
			border.visible=true;
			...
			headerContext.beginGradientFill(GradientType.LINEAR, headerColors, [1.0, 1.0], [0, 0xFF], matrix);

			GraphicsUtil.drawRoundRectComplex(headerContext, borderThickness, borderThickness, w, headerHeight, cornerRadius, cornerRadius, 0, 0);
			...
			fwdMonthButton.move(w - (monthSkinWidth + HEADER_WIDTH_PAD) + borderThickness, Math.round(borderThickness + (headerHeight - monthSkinHeight) / 2));
			backMonthButton.move(HEADER_WIDTH_PAD + borderThickness, Math.round(borderThickness + (headerHeight - monthSkinHeight) / 2));
		...
		override public function styleChanged(styleProp:String):void
		{
			...
			if (styleProp == null || styleProp == "styleName" || styleProp == "borderColor" || styleProp == "headerColor" || styleProp == "headerColors" || styleProp == "backgroundColor" || styleProp == "horizontalGap" || styleProp == "verticalGap" || styleProp == "backgroundAlpha")
			{
				invalidateDisplayList();
			}

			if (styleProp == null || styleProp == "styleName" || styleProp == "headerStyleName" && monthDisplay)
			{
		...
		private function getYearNavigationButtons():void
		{
			...
			if (!upYearButton)
			{
				...
				upYearButton.addEventListener(FlexEvent.BUTTON_DOWN, upYearButton_buttonDownHandler);
			...
			if (!downYearButton)
			{
				...
				downYearButton.addEventListener(FlexEvent.BUTTON_DOWN, downYearButton_buttonDownHandler);
		...
		mx_internal function setMonthWidth():void
		{
			...
			monthDisplay.width=longestMonthWidth * longestMonthUITextField.getExplicitOrMeasuredWidth() / measureText(longestMonthText).width;
		}
		...
		mx_internal function scrubTimeValue(value:Object):Object
		{
			...
			else if (value is Object)
			{
				...
				if (value.rangeStart)
				{
					range.rangeStart=new Date(value.rangeStart.getFullYear(), value.rangeStart.getMonth(), value.rangeStart.getDate());
				}

				if (value.rangeEnd)
				{
					range.rangeEnd=new Date(value.rangeEnd.getFullYear(), value.rangeEnd.getMonth(), value.rangeEnd.getDate());
				}
				return range;
			}
			return null;
		...
		private function upYearButton_buttonDownHandler(event:Event):void
		{
			...
			if ((selectableRange) && (dateGrid.selRangeMode == 1 || dateGrid.selRangeMode == 3))
			{
				var newDate:Object=CalendarLayout.getNewIncrementDate(displayedYear, displayedMonth, 1, 0);
				var testDate:Date=new Date(newDate.year, newDate.month, selectableRange.rangeEnd.getDate());
		...
		private function downYearButton_buttonDownHandler(event:Event):void
		{
			...
			if (selectableRange && (dateGrid.selRangeMode == 1 || dateGrid.selRangeMode == 2))
			{
				var newDate:Object=CalendarLayout.getNewIncrementDate(displayedYear, displayedMonth, -1, 0);
				var testDate:Date=new Date(newDate.year, newDate.month, selectableRange.rangeStart.getDate());
		...
		override public function invalidateProperties():void
		{
			...
			{
				dateGrid.setExtraMonthData(extraData);
			}
		}
		...
		public function set extraData(value:Array):void
		{
			_extraData=value;
			invalidateProperties();
		}
